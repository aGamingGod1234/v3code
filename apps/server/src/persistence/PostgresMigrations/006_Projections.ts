// Postgres port of SQLite migration 005_Projections — the core
// projection tables (projects/threads/messages/activities/sessions/turns/
// pending-approvals/state).
//
// Dialect deltas:
//   - `row_id INTEGER PRIMARY KEY AUTOINCREMENT` (projection_turns) →
//     `row_id BIGSERIAL PRIMARY KEY`.
//   - `is_streaming INTEGER NOT NULL` in `projection_thread_messages`
//     stays INTEGER (not BOOLEAN) — existing query code compares against
//     0/1 literals.
//   - `last_applied_sequence INTEGER NOT NULL` widened to BIGINT to
//     match the BIGSERIAL `orchestration_events.sequence` column it
//     tracks.
