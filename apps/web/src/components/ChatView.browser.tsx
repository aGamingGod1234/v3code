...
function toShellThread(thread: OrchestrationReadModel["threads"][number]) {
  return {
    ...
    orchestratorConfig: thread.orchestratorConfig,
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            worktreePath: null,
            env: {
              V3CODE_PROJECT_ROOT: "/repo/project",
            },
          });
          expect(openRequest?.env?.V3CODE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              V3CODE_PROJECT_ROOT: "/repo/project",
              V3CODE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        threads: createSnapshotForTargetUser({
          ...
          thread.id === THREAD_ID
            ? Object.assign({}, thread, {
                ...
                worktreePath: "/repo/.v3code/worktrees/existing",
              })
            : thread,
        ),
      },
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: V3 Code Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
