## [2026-05-11] - Orchestrator Audit PR Release Artifact

### What Was Implemented

- Built a Windows x64 desktop installer for the orchestrator audit branch.
- Added the new release `.exe`, blockmap, and refreshed Windows updater manifest under `release/`.

### Files Modified

- `release/V3-Code-0.0.25-v3.20260511.orchestrator-audit-x64.exe` - new Windows x64 installer artifact.
- `release/V3-Code-0.0.25-v3.20260511.orchestrator-audit-x64.exe.blockmap` - new updater blockmap for the installer.
- `release/latest.yml` - updated updater metadata for the new artifact.
- `PROJECT_LOG.md` - recorded the release artifact task.

### Assumptions Made (flag these for review)

- The release artifact version should follow the existing prerelease naming style as `0.0.25-v3.20260511.orchestrator-audit`.

### Known Issues / Deferred

- The installer is an unsigned local Windows build, matching the repo's existing local release artifact flow.

### Suggested Next Steps

- Use CI or a signing-capable release environment for a signed production installer when ready.

## [2026-05-11] - Orchestrator Audit Fixes

### What Was Implemented

- Audited the orchestrator data flow with sub-agents across server/runtime persistence, contracts/package wiring, and web/chat UX.
- Reduced orchestrated stream write amplification by coalescing implementation lane chunks and skipping thread summary refreshes for lane-only chunk activity.
- Made synthetic orchestrator task/lane activities deterministic and ordered, including failed task completion events when turn startup fails.
- Restarted orchestrated implementation sessions when the implementation provider changes, instead of reusing a mismatched provider session.
- Fixed sparse orchestrator settings persistence and normalized role defaults when a role provider changes.
- Tightened migrations and planning budget validation.
- Locked started orchestrated chats to the orchestrator provider UX, sent raw prompts for orchestrated turns, bounded lane panel scrolling, and removed NUL separators from composer draft keys.

### Files Modified

- `apps/server/src/orchestration/Layers/ProviderCommandReactor.ts` - handles provider changes, deterministic activity ids/sequences, and orchestrator task failure cleanup.
- `apps/server/src/orchestration/Layers/ProviderRuntimeIngestion.ts` - coalesces implementation lane chunks and emits deterministic task completion activity.
- `apps/server/src/orchestration/Layers/ProjectionPipeline.ts` - avoids expensive thread summary refreshes for agent lane chunk activity.
- `apps/server/src/persistence/Migrations/034_ProjectionThreadsOrchestratorSession.ts` - checks existing columns before SQLite migration changes.
- `apps/server/src/serverSettings.ts` and `packages/shared/src/serverSettings.ts` - persist orchestrator config atomically and normalize provider-specific role defaults.
- `packages/contracts/src/orchestrator-config.ts` - restricts planning budget to `auto` or a positive integer.
- `apps/web/src/components/ChatView.tsx`, `apps/web/src/components/ChatView.logic.ts`, `apps/web/src/components/chat/ProviderModelPicker.tsx`, and `apps/web/src/components/OrchestratedSession/*` - keep orchestrated sends and started orchestrated chats aligned with orchestrator mode.
- `apps/web/src/components/settings/OrchestratorSettings/*` and `apps/web/src/composerDraft.helpers.ts` - prevent invalid empty role/sub-agent model/name writes and remove embedded NUL separators.
- `apps/server/src/orchestration/Layers/ProviderCommandReactor.test.ts` and `apps/server/src/serverSettings.test.ts` - added focused regressions for provider switching and settings persistence.

### Assumptions Made (flag these for review)

- Gemini and custom orchestrator providers should remain configurable with fallback behavior until runtime adapters exist.
- Coalescing implementation lane chunks at a short threshold is preferable to one database/projector pass per assistant delta.

### Known Issues / Deferred

- The orchestrator still delegates implementation through the active provider runtime rather than spawning the full three-process server-side CLI set.
- Browser screenshot verification of the new settings/chat UI remains deferred until an authenticated local session is available.

### Suggested Next Steps

- Add provider adapters for Gemini/custom when their runtime integration is ready.
- Add browser coverage for Settings > Orchestrator and started orchestrated chat mode.

## [2026-05-11] - Orchestrated Session Foundation

### What Was Implemented

- Added generic orchestrator configuration contracts for Claude Code, Codex, Gemini, and custom providers with free-text model strings, provider-specific effort, and contextual mode values.
- Added session mode/config persistence through orchestration thread events, projections, SQLite/Postgres migrations, and WebSocket bootstrap flow.
- Added an internal `@v3tools/agent-harness` package with CLI process, routing, session, and task queue primitives for multi-agent orchestration.
- Routed orchestrated turns through the configured implementation role, projected orchestrator task/lane events, and marked task completion from provider turn completion.
- Added explicit runtime fallback handling for generic implementation providers that are configurable but not backed by an installed server adapter yet.
- Added Orchestrator as a chat provider option and a three-lane in-chat orchestrated session panel backed by activity events.
- Added Settings > Orchestrator for role slots, sub-agent definitions, fast mode, and planning budget.
- Added the Claude Sonnet assistant sub-agent definition.

### Files Modified

- `packages/contracts/src/orchestrator-config.ts` - added generic orchestrator config schemas and defaults.
- `packages/contracts/src/orchestration.ts` and `packages/contracts/src/settings.ts` - added session mode, thread config, and settings payload support.
- `apps/server/src/orchestration/*` and `apps/server/src/persistence/*` - persisted session mode/config and routed orchestrated turns through provider runtime events.
- `packages/agent-harness/*` - added the new internal package for CLI lifecycle, routing, and task state primitives.
- `apps/web/src/components/chat/*`, `apps/web/src/components/ChatView.tsx`, and composer draft/store files - added orchestrator provider selection and draft/session mode state.
- `apps/web/src/components/settings/OrchestratorSettings/*`, `apps/web/src/routes/settings.orchestrator.tsx`, and settings navigation/search files - added orchestrator configuration UI.
- `apps/web/src/components/OrchestratedSession/*` - added the in-chat three-lane session and task queue UI.
- `.claude/agents/sonnet-assistant.md` - added the assistant lane definition.
- `.gitignore` - allowed the tracked Claude assistant definition while keeping other Claude runtime files ignored.
- `PROJECT_LOG.md` - recorded this implementation.

### Assumptions Made (flag these for review)

- This repo's current `packages/contracts` package is the intended replacement for the requested `packages/mesh-contracts` paths.
- The current event-sourced projection tables replace the requested chats/user_preferences DB columns.
- Gemini and custom providers are persisted and configurable now; implementation dispatch announces a runtime fallback and routes through the thread's active provider until those adapters exist.

### Known Issues / Deferred

- The orchestrator system currently routes implementation through the existing provider runtime instead of launching three long-lived CLI subprocesses from the server path.
- The settings model dropdown uses native datalist free-text input rather than a custom searchable combobox.
- Authenticated browser screenshot coverage is still deferred; the production preview served, but the available pairing token was invalid by the final smoke pass.

### Suggested Next Steps

- Add provider adapter mappings for Gemini/custom when those runtimes are available.
- Add focused tests for orchestrated turn lifecycle events and the settings patch schema.
- Run browser screenshot coverage for the new orchestrator settings and chat lane UI in an authenticated local session.
