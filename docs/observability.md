...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.v3code/userdata/logs/server.trace.ndjson`).
...
```bash
export V3CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export V3CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export V3CODE_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export V3CODE_TRACE_MIN_LEVEL=Info
export V3CODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `V3CODE_OTLP_*`.

macOS app bundle example:

```bash
V3CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
V3CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
V3CODE_OTLP_SERVICE_NAME=t3-desktop \
...
```bash
tail -f "$V3CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$V3CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$V3CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- `V3CODE_OTLP_TRACES_URL` was not set
...
- `V3CODE_TRACE_FILE`: override trace file path
- `V3CODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `V3CODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `V3CODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `V3CODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `V3CODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `V3CODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `V3CODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `V3CODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `V3CODE_OTLP_SERVICE_NAME`: service name, default `t3-server`
